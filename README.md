...
给 [pi coding agent](https://pi.dev) 的输入框上方加一行常驻「常用提示词」按钮：**悬停高亮，点一下填进输入框**。

![demo](https://img.shields.io/badge/pi-package-blue)

```
        ┃                 ┃
        ┃  继续  总结  查bug  ┃   ← 悬停反白，点击填入输入框
        ┃                 ┃
  ╭─────────────────────────────────────────╮
  │ > 输入消息…                              │
  ╰─────────────────────────────────────────╯
...
| 操作 | 效果 |
| --- | --- |
| 鼠标点击按钮 | 把该提示词**填进输入框**（不发送），可继续补充后自己回车 |
| 鼠标悬停 | 按钮底色变成主题 accent 色（普通态是淡灰底） |

点击是「填入」而不是「发送」：输入框为空则直接填入，已有内容则换行追加 —— **不会吃掉你已经打了一半的字**。
agent 正在输出时也随时可点（只改输入框内容，不涉及消息投递）。
